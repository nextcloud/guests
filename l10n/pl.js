OC.L10N.register(
    "guests",
    {
    "Create guest account for {searchTerm}" : "Utwórz konto gościa dla {searchTerm}",
    "Access to this resource is forbidden for guests." : "Dostęp do zasobów jest zabroniony dla gości",
    "Invalid mail address" : "Nieprawidłowy adres email",
    "A user with that email already exists." : "Użytkownik o tym e-mailu już istnieje.",
    "A user with that name already exists." : "Użytkownik z taką nazwą już istnieje.",
    "User successfully created" : "Stworzono nowego użytkownika",
    "%s shared »%s« with you" : "%s Współdzielone »%s« z tobą",
    "Incoming share" : "Przychodzący udział",
    "Hey there," : "Hej tam,",
    "%s just shared »%s« with you." : "%s tylko udostępnia »%s« z Tobą.",
    "You can access the shared file by activating your guest account." : "Możesz uzyskać dostęp do udostępnionego pliku, aktywując konto gościa.",
    "After your account is activated you can view the share by logging in with %s." : "Po aktywowaniu konta możesz wyświetlić udział, logując się przy pomocy %s.",
    "The share will expire at %s." : "Ten zasób wygaśnie %s",
    "Activate account" : "Aktywuj konto",
    "View share" : "Wyświetl udział",
    "%s via %s" : "%s przez %s",
    "Couldn't send reset email. Please contact your administrator." : "Nie mogę wysłać maila resetującego. Skontaktuj się z administratorem.",
    "You can view the share by logging in with %s." : "Możesz wyświetlić udział, logując się przy pomocy %s.",
    "Guests" : "Goście",
    "Create guest accounts for easier collaboration" : "Utwórz konta dla gości dla łatwiejszej współpracy",
    "Create guest account for {fullName}" : "Utwórz konto gościa dla {fullName}",
    "Error while sharing\n" : "Błąd podczas udostępniania\n",
    "Error" : "Błąd",
    "Guest users are grouped under a virtual group in the user manager" : "Goście są grupowani w wirtualnej grupie w zarządzaniu użytkownikami",
    "Guest users can access mounted external storages" : "Użytkownicy, jako goście mogą uzyskać dostęp do zamontowanych zewnętrznych zasobów dyskowych ",
    "Limit guest access to an app whitelist" : "Ogranicz dostęp gości do białej listy aplikacji",
    "Reset" : "Resetuj",
    "Saved" : "Zapisano"
},
"nplurals=4; plural=(n==1 ? 0 : (n%10>=2 && n%10<=4) && (n%100<12 || n%100>14) ? 1 : n!=1 && (n%10>=0 && n%10<=1) || (n%10>=5 && n%10<=9) || (n%100>=12 && n%100<=14) ? 2 : 3);");
